...
def F_nos_th(Phi):
	...
	result[:,2]=(1/tau_hip)*(rho-NO_0)/NO_0#rho[ind_e[:,0],ind_e[:,1]]
	return result
...
def IP():
	...
	rho+=(NOS_Th[:,1].sum()/1000.)*dt_ip/size_T**2# divide by 1000 to get from production/s -> production/ms
...
path = sim_data_base_folder + "CF_dataset_instant_diff/"  #set path for saving sim.

if not os.path.isdir(path):   # create if not existent
	os.makedirs(path)

## Save connection fraction etc. as csv files
with open(path + "CF_eTOe.csv","a") as datafile:
	writer = csv.writer(datafile)
	writer.writerow(CF_eTOe)

with open(path + "CFB_eTOe.csv","a") as datafile:
	writer = csv.writer(datafile)
	writer.writerow(CFB_eTOe)

with open(path + "RC_CFB_eTOe.csv","a") as datafile:
	writer = csv.writer(datafile)
	writer.writerow(RC_CFB_eTOe)

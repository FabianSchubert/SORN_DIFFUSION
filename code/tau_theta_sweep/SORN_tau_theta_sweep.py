...
n_sweep = 50 # number of steps for sweeping through tau_hip

tau_hip_switch_times = np.linspace(t_switch,run_time,n_sweep+1) # times to subsequently increase tau_hip
tau_hip_value_list = np.linspace(2500.,50000.,n_sweep) # linspace of tau_hip values to sweep through
...
### Neuron Groups
G_e = NeuronGroup(N = N_e, model = noisylif, threshold ='V > Vt', reset=Vr_e, clock=defaultclock)  # excitatory group
G_i = NeuronGroup(N = N_i, model = noisylif, threshold ='V > Vt', reset=Vr_i, clock=defaultclock)  # inhibitory group
...
stdp_eTOe=ExponentialSTDP(C_eTOe, taupre, taupost, Ap, Ad, interactions='nearest',
						wmin=0, wmax=total_in_eTOe, update='additive',clock=defaultclock)
...
def IP():
	...
	if defaultclock.t >= t_switch:
		...
		## update tau_hip
		tau_hip = ((defaultclock.t >= tau_hip_switch_times[:-1])*(defaultclock.t < tau_hip_switch_times[1:])*tau_hip_value_list).sum()
		...
		NO_0+=dt_ip*(-NO_0+rho[ind_e[:,0],ind_e[:,1]].mean())/(20000.)  # adapt target NO concentration during non-diff. IP via a linear filter
...
path = sim_data_base_folder + "tau_theta_sweep/"  #set path for saving sim.
...
try:
	# save threshold recordings and tau_hip
	np.save(path + "thresholds_e.npy",np.array(MVt_e.values))
	
	np.save(path + "tau_hip_value_list.npy",tau_hip_value_list)
	
	np.save(path + "tau_hip_switch_times.npy",tau_hip_switch_times)

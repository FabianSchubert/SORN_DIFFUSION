...
### Pre- and post-normalization recording arrays
W_eTOe_prenorm = np.ndarray((int(N_e*run_time/slow_time),N_e))
W_eTOe_postnorm = np.ndarray((int(N_e*run_time/slow_time),N_e))
...
def synnorm_eTOe():
	global W_eTOe_prenorm
	global W_eTOe_postnorm
	
	W_eTOe_prenorm[int(defaultclock.t/slow_time)*N_e:(int(defaultclock.t/slow_time)+1)*N_e,:] = C_eTOe.W.todense()
	...
	for i in xrange(N_e):
		...
		if not sum_in == 0:
			...
			"""
	W_eTOe_postnorm[int(defaultclock.t/slow_time)*N_e:(int(defaultclock.t/slow_time)+1)*N_e,:] = C_eTOe.W.todense()
...
path = sim_data_base_folder + "complete_diff_prepostnorm/" #set path for saving sim. data

if not os.path.isdir(path): # create if not existent
	os.makedirs(path)

### Prepare weight matrix recordings for saving -> to sparse matrix
W_eTOe_prenorm_sparse = sparse.csr_matrix(W_eTOe_prenorm)
W_eTOe_postnorm_sparse = sparse.csr_matrix(W_eTOe_postnorm)

try:
	## pickle recorded data/ save as .npy array
	pickle.dump(MS_e.spiketimes,open(path + "spiketimes_e.p","wb"))
        pickle.dump(MS_i.spiketimes,open(path + "spiketimes_i.p","wb"))
        pickle.dump(MVt_e.values,open(path + "thresholds_e.p","wb"))
        pickle.dump(MVt_i.values,open(path + "thresholds_i.p","wb"))
        pickle.dump(CF_eTOe,open(path + "CF_eTOe.p","wb"))
        pickle.dump(CFB_eTOe,open(path + "CFB_eTOe.p","wb"))
        pickle.dump(RC_CFB_eTOe,open(path + "RC_CFB_eTOe.p","wb"))
        pickle.dump(rho_rec,open(path + "rho_rec.p","wb"))
        pickle.dump(Ca_rec,open(path + "Ca_rec.p","wb"))
        pickle.dump(Ca_rec_tot,open(path + "Ca_rec_tot.p","wb"))
        pickle.dump(nNOS_rec,open(path + "nNOS_rec.p","wb"))
        pickle.dump(nNOS_rec_tot,open(path + "nNOS_rec_tot.p","wb"))
        pickle.dump(rho_rec_tot,open(path + "rho_rec_tot.p","wb"))
        pickle.dump(rho_rec_N_e,open(path + "rho_rec_N_e.p","wb"))
        pickle.dump(X_e,open(path + "X_e.p","wb"))
        pickle.dump(X_i,open(path + "X_i.p","wb"))
        pickle.dump(ind_e,open(path + "ind_e.p","wb"))
        pickle.dump(ind_i,open(path + "ind_i.p","wb"))
       	np.save(path + "W_eTOe.npy",C_eTOe.W.todense())
       	np.save(path + "W_eTOi.npy",C_eTOi.W.todense())
       	np.save(path + "W_iTOe.npy",C_iTOe.W.todense())
       	np.save(path + "W_iTOi.npy",C_iTOi.W.todense())
        np.save(path + "W_eTOe_prenorm.npy",W_eTOe_prenorm_sparse)
        np.save(path + "W_eTOe_postnorm.npy",W_eTOe_postnorm_sparse)
      
except:
        print("Error - Data was not saved.")
        pdb.set_trace() # catch into a command line if saving fails, so that data is not lost instantly

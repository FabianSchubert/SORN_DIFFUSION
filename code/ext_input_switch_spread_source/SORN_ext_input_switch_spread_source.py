...
## Radius of the circular NO sources
R_source = 50.

## corresponding radius in units of the numeric grid
n_max_source = int(R_source/h)

kernel_ind_source = [[],[]]

## list of grid indices to be used for NO influx
...
area_kernel = 1.*kernel_ind_source.shape[1]*hsqr # total area of the source
...
print("initializing E->E connections...")
...
print("initializing E->I connections...")
...
print("initializing I->E connections...")
...
print("initializing I->I connections...")
...
### Ext. Poisson Input Groups to G_e

G_e_in_0 = G_e.subgroup(N_ext)
G_e_in_1 = G_e.subgroup(N_ext)

G_ext_0 = PoissonInput(G_e_in_0,N_ext, 0.*hertz, ext_weight, 'V') ### initialize with zero frequency
G_ext_1 = PoissonInput(G_e_in_1,N_ext, 0.*hertz, ext_weight, 'V')
...
def IP():
	...
	for k in xrange(N_e):
		...
		rho[kernel_temp[0,:],kernel_temp[1,:]]+=(NOS_Th[k,1]/1000.)*dt_ip/area_kernel # divide by 1000 to get from production/s -> production/ms
...
MS_e_inp_0 = SpikeMonitor(G_e_in_0)
MS_e_inp_1 = SpikeMonitor(G_e_in_1)
...
path = sim_data_base_folder + "ext_input_switch_spread_source/"  #set path for saving sim.

if not os.path.isdir(path):   # create if not existent
	os.makedirs(path)

try:
	# pickle data/ save as .npy array  
	pickle.dump(MS_e.spiketimes,open(path+"spiketimes_e.p","wb"))
	pickle.dump(MS_i.spiketimes,open(path+"spiketimes_i.p","wb"))
	pickle.dump(MS_e_inp_0.spiketimes,open(path+"spiketimes_e_inp_0.p","wb"))
	pickle.dump(MS_e_inp_1.spiketimes,open(path+"spiketimes_e_inp_1.p","wb"))

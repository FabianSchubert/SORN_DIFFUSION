...
def F_nos_th(Phi):
	...
	result[:,2]=(1/tau_hip)*(rho-NO_0)/NO_0
	return result
...
# Flag for switching external input
...
def IP():
	...
	rho+=(NOS_Th[:,1]/1000.).sum()*dt_ip/size_T**2
	...
	if defaultclock.t >= t_switch:
		...
		NO_0+=dt_ip*(-NO_0+rho)/(20000.) # adapt target NO concentration during non-diff. IP via a linear filter
...
def struct_new_synapses():
	...
	for i in xrange(n_new): # throw randoms to populate connections; better method to come
		...
		while not addition_allowed:
			new_i = randint(0, high=N_e)
			new_j = randint(0, high=N_e)
			if rand() < P_eTOe[new_i,new_j]: ## replace condition by "True" to get uniform spatial topology
				if not new_i == new_j and C_eTOe[new_i,new_j] == 0:
					addition_allowed = True
...
path = sim_data_base_folder + "complete_instant_diff_long/" #set path for saving sim. data
...
try:
	...
	pickle.dump(MS_e.spiketimes,open(path + "spiketimes_e.p","wb"))
        ...
        np.save(path + "W_eTOe_record.npy",W_eTOe)
        pickle.dump(times_W,open(path + "times_W.p","wb"))

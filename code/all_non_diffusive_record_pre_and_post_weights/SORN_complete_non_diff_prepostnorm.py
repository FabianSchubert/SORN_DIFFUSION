...
def synnorm_eTOe():
	global W_eTOe_prenorm # record weights before and after normalization
...
path = sim_data_base_folder + "complete_non_diff_prepostnorm/" #set path for saving sim. data

if not os.path.isdir(path):  # create if not existent
	os.makedirs(path)

W_eTOe_prenorm_sparse = sparse.csr_matrix(W_eTOe_prenorm) # prepare weight recordings for saving -> to sparse

...
# This script plots the time course of the population firing rate of 3 excitatory subpopulations upon external Poisson input, using non-point-like, circular nNOS sources.

## Simulation scripts required to run before this script:
# -ext_input_spread_diff_source

# where to find data and where to save figure
folder = sim_data_base_folder + "ext_input_switch_spread_source/"
savefolder = plots_base_folder
plot_filename = "external_input_switch_spread_source"
...
f_full = frequ_bin_time(spt_full,500.,2000.,1500).mean(axis=1)
f_e_inp_0 = frequ_bin_time(spt_e_inp_0,500.,2000.,1500).mean(axis=1)
f_e_inp_1 = frequ_bin_time(spt_e_inp_1,500.,2000.,1500).mean(axis=1)
